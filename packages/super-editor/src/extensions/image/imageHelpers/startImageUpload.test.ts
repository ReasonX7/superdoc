...
import * as processModule from './processUploadedImage';
import {
  checkAndProcessImage,
  replaceSelectionWithImagePlaceholder,
  uploadAndInsertImage,
} from './startImageUpload';
import { findPlaceholder, removeImagePlaceholder } from './imageRegistrationPlugin';
import * as relsMutationModule from '@core/parts/adapters/relationships-mutation';
...
describe('uploadAndInsertImage collaboration branch (isolated)', () => {
  ...
  it('calls addImageToCollaboration when ydoc is provided', async () => {
    ...
    vi.doMock('./imageRegistrationPlugin', () => ({
      findPlaceholder: () => 0,
      removeImagePlaceholder: (_state, tr) => tr,
      addImagePlaceholder: vi.fn(),
    }));

    vi.doMock('@core/parts/adapters/relationships-mutation', () => ({
      findOrCreateRelationship: vi.fn(() => 'rId100'),
    }));

    const { uploadAndInsertImage } = await import('./startImageUpload');
