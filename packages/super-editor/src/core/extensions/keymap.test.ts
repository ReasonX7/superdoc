...
const setupKeymap = async ({ isMacOS, isIOS }) => {
  vi.resetModules();
  vi.doMock('../utilities/isMacOS', () => ({ isMacOS: () => isMacOS }));
  vi.doMock('../utilities/isIOS', () => ({ isIOS: () => isIOS }));

  const { Keymap } = await import('./keymap');
  const { getExtensionConfigField } = await import('../helpers/getExtensionConfigField');
