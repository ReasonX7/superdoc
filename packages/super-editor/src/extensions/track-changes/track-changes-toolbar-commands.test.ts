...
vi.mock('../comment/comments-plugin', () => ({
  CommentsPluginKey: {
    getState: vi.fn(),
  },
}));

vi.mock('./permission-helpers', () => ({
  collectTrackedChanges: vi.fn(),
}));

vi.mock('./trackChangesHelpers/getTrackChanges', () => ({
...
describe('Track Changes Shared Resolution Commands', () => {
  ...
  beforeEach(async () => {
    ...
    const { CommentsPluginKey } = await import('../comment/comments-plugin');
    const { collectTrackedChanges } = await import('./permission-helpers');
    const { getTrackChanges } = await import('./trackChangesHelpers/getTrackChanges');
