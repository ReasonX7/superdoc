...
import { createMockEditor, createMockContext, assertMenuSectionsStructure, ContextMenuConfigs } from './testHelpers';
...
describe('menuItems', () => {
  ...
  beforeEach(async () => {
    ...
    const { selectionHasNodeOrMark } = await import('../../cursor-helpers');
    const { isTrackedChangeActionAllowed } = await import('@extensions/track-changes/permission-helpers');
