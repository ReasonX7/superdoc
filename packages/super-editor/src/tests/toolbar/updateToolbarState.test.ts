...
vi.mock('@core/helpers/getActiveFormatting', () => ({
...
vi.mock('@helpers/isInTable', () => ({
  isInTable: vi.fn().mockImplementation(() => false),
}));

vi.mock('@extensions/linked-styles/linked-styles', () => ({
  getQuickFormatList: vi.fn(),
}));

vi.mock(import('@helpers/index'), async (importOriginal) => {
...
describe('updateToolbarState', () => {
  ...
  beforeEach(async () => {
    ...
    const { getActiveFormatting } = await import('@core/helpers/getActiveFormatting');
    const { isInTable } = await import('@helpers/isInTable');
    const { getQuickFormatList } = await import('@extensions/linked-styles/linked-styles');
    const { collectTrackedChanges, isTrackedChangeActionAllowed } = await import(
      '@extensions/track-changes/permission-helpers'
    );
