...
import { initTestEditor } from '../helpers/helpers';
import { createFootnoteElement, prepareFootnotesXmlForExport } from '@converter/v2/exporter/footnotesExporter';
import { importFootnoteData } from '@converter/v2/importer/documentFootnotesImporter';
...
describe('customMarkFollows attribute', () => {
  it('imports footnoteReference with customMarkFollows attribute', async () => {
    ...
    const { createDocumentJson } = await import('@converter/v2/importer/docxImporter');
...
describe('bootstrapped notes part produces unique ids', () => {
  it('creates separator=-1, continuationSeparator=0, first real note=1 with no duplicates', async () => {
    ...
    const { bootstrapNotesPart, getNotesConfig, addNoteElement } = await import(
      '@core/parts/adapters/notes-part-descriptor'
    );
