...
vi.mock('../../core/helpers/editorSurface', async () => {
  const actual = await vi.importActual('../../core/helpers/editorSurface');
  return {
    ...actual,
...
describe('ContextMenu.vue', () => {
  ...
  beforeEach(async () => {
    ...
    const { getItems } = await import('../menuItems');
    const { getEditorContext } = await import('../utils');
  ...
  describe('menu items rendering', () => {
    ...
    it('should keep selection when right-click happens inside the active selection', async () => {
      ...
      const { moveCursorToMouseEvent } = await import('../../cursor-helpers');
