...
import * as contentProcessor from '../helpers/contentProcessor';

vi.mock('../helpers/contentProcessor');
...
describe('insertContent (integration) list export', () => {
  ...
  const setupEditor = async () => {
    ...
    vi.resetModules();
    vi.doUnmock('../helpers/contentProcessor');

    // Cache helpers and DOCX data on first call
    if (!helpers) {
      helpers = await import('../../tests/helpers/helpers');
    }
    if (!cachedDocxData) {
      cachedDocxData = await helpers.loadTestDataForEditorTests('blank-doc.docx');
    }
    if (!exportHelpers) {
      exportHelpers = await import('../../tests/export/export-helpers/index');
    }
