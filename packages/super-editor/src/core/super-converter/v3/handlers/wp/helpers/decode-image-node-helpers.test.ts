...
} from '@converter/v3/handlers/wp/helpers/decode-image-node-helpers';
import * as helpers from '@converter/helpers';
import * as annotationHelpers from '@converter/v3/handlers/w/sdt/helpers/translate-field-annotation';
...
vi.mock('@converter/v3/handlers/w/sdt/helpers/translate-field-annotation', () => ({
  prepareTextAnnotation: vi.fn(() => ({ type: 'text', text: 'annotation' })),
}));

vi.mock(import('@core/helpers/index'), async (importOriginal) => {
  const actual = await importOriginal();
  return {
    ...actual,
