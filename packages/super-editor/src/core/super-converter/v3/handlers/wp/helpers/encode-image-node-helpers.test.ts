...
import { handleImageNode, getVectorShape } from './encode-image-node-helpers';
import { emuToPixels, polygonToObj, rotToDegrees } from '@converter/helpers';
import { extractFillColor, extractStrokeColor, extractStrokeWidth, extractLineEnds } from './vector-shape-helpers';
import { convertTiffToPng } from './tiff-converter';

vi.mock('@converter/helpers', async (importOriginal) => {
...
vi.mock('./tiff-converter', async (importOriginal) => {
  const actual = await importOriginal();
  return {
    ...actual,
